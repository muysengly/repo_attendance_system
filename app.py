...
all_directories_and_embs = []  # list of all embeddings
...
def load_database():
    ...
    global group_student_files, group_student_embs, group_name, all_directories_and_embs

    group_student_files = pickle.load(open("resource/group_student_files.pkl", "rb"))
    current_group_student_files = scan_directory("database")
    ...
    if group_student_files:
        ...
        all_directories_and_embs = gen_name_embs(group_student_embs[group_name])
...
tmp_app = QApplication([])
tmp_msg = QMessageBox()
tmp_msg.setWindowTitle("Database")
tmp_msg.setWindowIcon(QIcon("./resource/mu_logo.png"))
tmp_msg.setIcon(QMessageBox.Information)
tmp_msg.setText("Loading Database ...")
tmp_msg.setWindowFlags(tmp_msg.windowFlags() | Qt.WindowStaysOnTopHint)
tmp_msg.setStandardButtons(QMessageBox.NoButton)
tmp_msg.show()
tmp_app.processEvents()
load_database()
tmp_msg.close()
del tmp_app, tmp_msg
########## __________ ##########
            ...
            if len(self.faces) > 0:
                for face in self.faces:
                    ...
                    if (not all(len(x) == 0 for x in list(group_student_files.keys()))) and (not all(len(x) == 0 for x in list(group_student_files[group_name].values()))):
                        ...
                        # score for all names
                        score_all = []
                        for name, embs in all_directories_and_embs:
                            high_score = 0
                            for emb in embs:
                        ...
                        if np.max(np.array(score_all)) > similarity_threshold:
                            ...
                            cv2.putText(img=frame, text=f"{all_directories_and_embs[np.argmax(np.array(score_all))][0]} {100*np.max(np.array(score_all)):.0f}%", org=(box[0], box[1] - 10), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 255, 0), thickness=2)
                            ...
                            if (all_directories_and_embs[np.argmax(np.array(score_all))][0] not in self.data_attd) and (self.frame_count % self.SKIP_FRAMES == 0):

                                self.col_data[np.argmax(np.array(score_all))] = [self.col_data[np.argmax(np.array(score_all))], time.strftime("%H:%M:%S")]
                                self.data_attd.append(all_directories_and_embs[np.argmax(np.array(score_all))][0])
                                ...
                                cv2.putText(img=tmp_frame, text=f"{all_directories_and_embs[np.argmax(np.array(score_all))][0]} {100*np.max(np.array(score_all)):.0f}%", org=(box[0], box[1] - 20), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 255, 0), thickness=2)

                                # save image with name
                                cv2.imwrite(f"log/log_{group_name}_{all_directories_and_embs[np.argmax(np.array(score_all))][0]}_{date.today().strftime('%Y_%m_%d')}_{time.strftime('%H_%M_%S')}.jpg", tmp_frame)
                        else:
                            cv2.rectangle(img=frame, pt1=(box[0], box[1]), pt2=(box[2], box[3]), color=(0, 0, 255), thickness=2)
                            cv2.putText(img=frame, text=f"{all_directories_and_embs[np.argmax(np.array(score_all))][0]} {100*np.max(np.array(score_all)):.0f}%", org=(box[0], box[1] - 10), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 255), thickness=2)
...
def f_reset():
    ...
    global all_directories_and_embs
...
########## combobox group ##########
def f_group_change():
    global group_name, all_directories_and_embs
...
def f_capture():
    ...
    if selected:
        ...
        win.listView_init.clearSelection()
        win.listView_attd.clearSelection()
        ...
        # keep the current group name
        group_name = win.comboBox_group.currentText()
        all_directories_and_embs = gen_name_embs(group_student_embs[group_name])

        # reinit list view
        # if group_student_files:  # if database is not empty
        #     win.col_data = list(group_student_files[group_name].keys())
        #     win.data_init = list(group_student_files[group_name].keys())
        # else:
        #     win.col_data = []
        #     win.data_init = []
        # win.model_init = QStringListModel()
