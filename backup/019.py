...
from PyQt5.QtWidgets import QMainWindow, QMessageBox, QApplication, QProgressDialog
...
os.system("pyuic5 -x gui_v_1_19.ui -o gui_v_1_19.py")
from gui_v_1_19 import Ui_MainWindow
                    ...
                    if (not all(len(x) == 0 for x in list(group_student_files.keys()))) and (not all(len(x) == 0 for x in list(group_student_files[group].values()))):
                        ...
                        if np.max(np.array(score_all)) > THRESHOLD:
                            ...
                            if all_dirs_embs[np.argmax(np.array(score_all))][0] not in self.data_attd:
                                ...
                                if not os.path.exists(f"log/{group}/{all_dirs_embs[np.argmax(np.array(score_all))][0]}"):
                                    os.makedirs(f"log/{group}/{all_dirs_embs[np.argmax(np.array(score_all))][0]}")

                                cv2.imwrite(f"log/{group}/{all_dirs_embs[np.argmax(np.array(score_all))][0]}/log_{date.today().strftime('%Y_%m_%d')}_{time.strftime('%H_%M_%S')}.jpg", frame)
                        else:
...
def f_save():
    ...
    if (not all(len(x) == 0 for x in list(group_student_files.keys()))) and (not all(len(x) == 0 for x in list(group_student_files[group].values()))):
        ...
        if not os.path.exists(f"result/{group}"):
            os.makedirs(f"result/{group}")

        # save to csv
        with open(f"result/{group}/att_{date.today().strftime("%Y_%m_%d")}_{time.strftime("%H_%M_%S")}.csv", "w", newline="", encoding="utf-8") as file:
...
win.label_version.setText(f"Version: 1.19")
