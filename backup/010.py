...
# fa = FaceAnalysis(name="buffalo_l", root=os.getcwd())
# fa = FaceAnalysis(name="buffalo_s", root=os.getcwd())
fa = FaceAnalysis(name="buffalo_sc", root=os.getcwd())
...
def compare_faces_cosine(emb1, emb2, threshold=0.75):
    ...
    def paintEvent(self, event):
        ...
        if len(faces) > 0:
            for face in faces:
                ...
                if np.max(np.array(tmp_score_remain)) > 0.7:

                    # attendant data
                    self.col_data.append([tmp_names_emb[np.argmax(np.array(tmp_score_remain))][0], date.today().strftime("%Y-%m-%d"), time.strftime("%H:%M:%S")])

                    self.data_attd.append(tmp_names_emb[np.argmax(np.array(tmp_score_remain))][0])
                    self.model_attd.setStringList(self.data_attd)
                    self.listView_attd.setModel(self.model_attd)

                    tmp_names_emb.pop(np.argmax(np.array(tmp_score_remain)))
                ...
                # for attended faces
                elif np.max(np.array(tmp1_score_all)) > 0.7:

                    if len(self.data_attd) > 0:
                        for i in range(len(self.data_attd)):
                            if self.data_attd[i] == names_emb[np.argmax(np.array(tmp1_score_all))][0]:
                                cv2.rectangle(frame, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)
                                cv2.putText(img=frame, text=f"{names_emb[np.argmax(np.array(tmp1_score_all))][0]} Attended", org=(box[0], box[1] - 10), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 255, 0), thickness=2)
                                cv2.putText(img=frame, text=f"Score: {np.max(np.array(tmp1_score_all)):.2f}", org=(box[0], box[3] + 20), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 255, 0), thickness=2)

                # for unknown faces
                else:
                    cv2.rectangle(frame, (box[0], box[1]), (box[2], box[3]), (0, 0, 255), 2)
                    cv2.putText(img=frame, text=f"{tmp_names_emb[np.argmax(np.array(tmp_score_remain))][0]} ???", org=(box[0], box[1] - 10), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 255), thickness=2)
                    cv2.putText(img=frame, text=f"Score: {np.max(np.array(tmp_score_remain)):.2f}", org=(box[0], box[3] + 20), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 255), thickness=2)
        ...
        q_image = QImage(image.data, self.SIZE, self.SIZE, QImage.Format.Format_RGB888)
...
def f_save():
    ...
    with open(f"Attendant_{date.today().strftime("%Y_%m_%d")}_{time.strftime("%H_%M_%S")}.csv", "w") as f:
        for item in win.col_data:
            f.write("%s\n" % item)

    # show message box
    msg = QMessageBox()
    msg.setWindowIcon(QIcon("./logo/itc_logo.png"))
