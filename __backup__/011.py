        ...
        if len(faces) > 0:
            for face in faces:
                ...
                for name, embs in tmp_names_emb:
                    tmp_high_score = 0
                    for emb in embs:
                        similarity_score = compare_faces_cosine(face.embedding, emb)
                ...
                if np.max(np.array(tmp_score_remain)) > THRESHOLD:
                    ...
                    # attendant data
                    self.col_data.append(f"{tmp_names_emb[np.argmax(np.array(tmp_score_remain))][0]}, {date.today().strftime("%Y-%m-%d")}, {time.strftime("%H:%M:%S")}")
                    ...
                    if len(self.data_attd) > 0:
                        for i in range(len(self.data_attd)):
                            if self.data_attd[i] == names_emb[np.argmax(np.array(tmp1_score_all))][0]:
                                ...
                                cv2.putText(
                                    img=frame,
                                    text=f"{names_emb[np.argmax(np.array(tmp1_score_all))][0]} {100*np.max(np.array(tmp1_score_all)):.0f}%",
                                    org=(box[0], box[1] - 10),
                ...
                # for unknown faces
                    ...
                    cv2.putText(
                        img=frame,
                        text=f"{tmp_names_emb[np.argmax(np.array(tmp_score_remain))][0]} {100*np.max(np.array(tmp_score_remain)):.0f}%",
                        org=(box[0], box[1] - 10),
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale=0.5,
                        color=(0, 0, 255),
...
def f_save():
    ...
    with open(f"csv/attendant_{date.today().strftime("%Y_%m_%d")}_{time.strftime("%H_%M_%S")}.csv", "w") as f:
